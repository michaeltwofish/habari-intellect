...
function attachNavEvents(parent, myClass, setActive, hoverSpeed, style) {
	$(parent + " ." + myClass).mouseover(function() {
		// create pseudo-link
		$(this).append('<div class="js-' + myClass + '"></div>');
		// either slide or fade, depending on the style value
		if (style == "slide") {
			// slide down the pseudo-link
			$("div.js-" + myClass).css({display:"none"}).slideDown(hoverSpeed);
		} else {
			// fade in the pseudo-link
			$("div.js-" + myClass).css({display:"none"}).fadeIn(hoverSpeed);
		}
	}).mouseout(function() {
		// either slide or fade, depending on the style value
		if (style == "slide") {
			// slide up & destroy pseudo-link
			$("div.js-" + myClass).slideUp(hoverSpeed, function() {
				$(this).remove();
			});
		} else {
			// fade out & destroy pseudo-link
			$("div.js-" + myClass).fadeOut(hoverSpeed, function() {
				$(this).remove();
			});
		}
	});
	...
	if (setActive) {
		$(parent + " ." + myClass).mousedown(function() {
			$("div.js-" + myClass).attr("class", "js-" + myClass + "-click");
		}).mouseup(function() {
			$("div.js-" + myClass + "-click").attr("class", "js-" + myClass);
		});
	}
}
